...
public class HandDetector {
    ...
    public HandDetector(String hsvFnm, int width, int height) {
        ...
        msgFont = new Font("SansSerif", Font.BOLD, 18);//For font in Java.lanfg for jpanel
    ...
    private void setHSVRanges(String fnm) /* read in three lines to set the lower/upper HSV ranges for the user's glove. 
      ...
      */ {
        try {
            ...
            System.out.println(hueLower + " " + hueUpper + " " + satLower + " " + satUpper + " " + briLower + " " + briUpper);
        } catch (Exception e) {
     ...
     */ {
        ...
        cvCvtColor(scaleImg, hsvImg, CV_BGR2HSV_FULL);
     ...
     */ {
        ...
        if (m00 != 0) {   // calculate center
            ...
            System.out.println("cogPt = " + cogPt);

            try {

                Robot robot = new Robot();
                robot.mouseMove((int) (xCenter*1.5), (int) (yCenter*1.5));
            } catch (Exception e) {
            }
        }
    ...
    private void reduceTips(int numPoints, Point[] tipPts, Point[] foldPts, float[] depths) /* Narrow in on 'real' 
     * finger tips by ignoring shallow 
     * defect depths, and tips
    ...
    private void performGestures() {
        ...
        int len = fingerTips.size();

        boolean indexDetected = false;
        boolean thumbDetected = false;
        boolean midDetected = false;
        boolean ringDetected = false;
        boolean littleDetected = false;
        ...
        for (int i = 0; i < len; i++) {
            ...
            if (namedFingers.get(i) == FingerName.INDEX) {
                //Dont do anything
                indexDetected = true;
            }
            if (namedFingers.get(i) == FingerName.LITTLE) {
                //Dont do anything
                littleDetected = true;


            }
            if (namedFingers.get(i) == FingerName.MIDDLE) {
                //Dont do anything
                midDetected = true;
            }
            if (namedFingers.get(i) == FingerName.RING) {
                //Dont do anything
                ringDetected = true;
            }

            if (namedFingers.get(i) == FingerName.THUMB) {
                //Dont do anything
                thumbDetected = true;
        ...
        if(thumbDetected&&indexDetected&&!littleDetected&&!ringDetected&&!midDetected)
        {
            try {
                Robot r=new Robot();
